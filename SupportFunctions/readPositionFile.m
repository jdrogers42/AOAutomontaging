...
if strcmp(device_mode, 'multi_modal')
    ...
    if strcmp(posFileLoc(end-3:end), '.csv')
        ...
        % file ids.
        padlength = length(C{1,1}) % Pad with the same number of zeros we started with (JDR 20211011)
        C(:,1) = cellfun(@(x) ['_' num2str( str2double(x),['%0' num2str(padlength) '.0f']) '_'], C(:,1),'UniformOutput',false);
